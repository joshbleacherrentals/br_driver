...
| Race risk from parallel watch callbacks | Yes, confirmed in the library's current code                           | None — a single claim gatekeeper (bounded, `MAX_CONCURRENT_UPLOADS`-way concurrent execution; exclusivity comes from an in-memory claim reservation, not from running one row at a time) |
...
- **One worker run for the whole queue, with bounded concurrency** — there is still exactly one run at a time (overlapping triggers join the single in-flight run via an `isRunning` flag, unlike the current per-id guard, which doesn't protect against parallel watch cycles), but that run drains through a small pool of lanes rather than a single cursor: `MAX_CONCURRENT_UPLOADS = 3` uploads may be in flight at once.
  ...
  Crucially, **exclusivity no longer comes from running one row at a time — it comes from the claim**. `claimNext` is a plain `SELECT`, and nothing beneath the app layer stops two lanes reading the same not-yet-reserved row, so "pick a row" and "reserve it" are performed as one indivisible step behind an async lock (`withClaimLock` in `photoUploadService.ts`). Once a row is reserved it is invisible to every subsequent claim.

  **The reservation is in memory, not a database write.** It used to be `upload_status = 'uploading'`, persisted before the attempt and overwritten by the terminal outcome afterwards — two synced writes per photo per attempt. These are ordinary PowerSync tables, so every reservation became a CRUD operation queued for Postgres: measured on a real device, a drain at ~87 photos/min produced ~174 CRUD ops/min against ~55-76 ops/min of throughput, and the resulting backlog (peaking at 7.5 minutes) delayed *every other write the driver made*, because PowerSync uploads its CRUD queue strictly in order. A new damage report, an accepted trip or a finished inspection was written locally in a second and then sat behind thousands of photo status-updates.

  Nothing was bought with that write. Exclusivity here is scoped to one process, and a local SQLite status column never provided more than that — two processes reading `pending` would both write `uploading` and both upload. The service's `inFlightRows` ledger enforces exactly the same guarantee at exactly the same scope, inside the same lock, and `claimNext` consults it through an `isReserved` predicate (the one filter that cannot be pushed into SQL). What the write *additionally* bought was durability of the reservation across process death — which is the property §14 exists to undo, not preserve: an app killed mid-upload left a row stranded in `uploading` for hours. An in-memory reservation simply evaporates with the process, leaving the row at `pending`/`failed` with its original `attempts`, i.e. ordinary work for the next launch. §14's sweep stays regardless, because devices in the field still carry rows stranded by the old behaviour.

  The one case that needs care is the terminal write failing outright — §14's retries *and* its guaranteed-`failed` fallback. The row is then unchanged in the database, so releasing the reservation would hand it straight back to the next lane and re-upload the same photo in a loop with backoff never advancing. `persistUploadEvent` therefore reports whether *some* outcome was recorded, and the service keeps its claim when nothing was: the row is parked for the rest of the session and reclaimed as ordinary work on the next launch. Note this leaves no stuck row behind — unlike the `uploading` write it replaces, whose equivalent failure stranded the row in the database itself.
...
None of this widens `UNRESOLVED_STATUSES`. `uploading` stays out of the claim path, and the sweep is how a row already stuck there stops being a trap.

**Update — the queue no longer writes `uploading` at all.** The mid-attempt reservation moved into the service's in-memory claim ledger (§10), so the failure mode this section was written for cannot be created any more: an app killed mid-upload now leaves the row exactly where it was, retryable. Layers 1 and 2 remain — they protect the *terminal* write, which still has to reach the database — and layer 3 remains too, because every device that ran the previous behaviour may still be carrying rows stranded in `uploading`. It is now a migration mechanism rather than a standing backstop, and the in-flight check it consults is `PhotoUploadService.isRowInFlight`, which is also the reason that method is public: the database can no longer answer "is a lane working on this row".

The §6 recovery pass consults it for the same reason. A row mid-attempt now reads as ordinary `pending`/`failed` work, so `verifyAgainstBucket` would otherwise bucket-check a photo whose upload is still in flight — bannering it as lost, or writing `uploaded` underneath the lane whose own terminal write then lands on top and marks a delivered photo `failed`.
