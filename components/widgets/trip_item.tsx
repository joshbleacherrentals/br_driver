...
export default function TripItem({ workTracker, onAccept, onStartTrip, onSkip, onArrived, onStartInspection }: TripItemProps) {
  ...
  const formatPay = (cents: number | null) => cents === null ? '' : `$${(cents / 100).toFixed(2)}`;
  ...
  return (
    <View style={styles.card}>
      ...
      {status === 'pickup_inspection' && (
        <TouchableOpacity style={styles.inspectionButton} onPress={() => onStartInspection?.(workTracker.id, 'pickup')}>
      ...
      {status === 'dropoff_inspection' && (
        <TouchableOpacity style={styles.inspectionButton} onPress={() => onStartInspection?.(workTracker.id, 'dropoff')}>
