      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.header}>
          <View style={styles.headerLeft}>
            ...
            {/* NVIS PDF button — shown only when bleacher has an NVIS PDF */}
            <NvisPdfButton
        ...
        <View style={styles.section}>
          ...
          <TouchableOpacity onPress={() => openInMaps(pickupAddress ? `${pickupAddress.street}, ${pickupAddress.city}, ${pickupAddress.state_province}, ${pickupAddress.zip_postal}` : undefined)}>
            <Text style={styles.addressText}>{pickupAddress ? pickupAddress.street : 'Address not set'}</Text>
          </TouchableOpacity>
          {workTracker.pickup_time && <Text style={styles.detailText}>Time: {formatTime(workTracker.pickup_time)}</Text>}
          {workTracker.pickup_poc && <Text style={styles.detailText}>POC: {workTracker.pickup_poc}</Text>}
          {workTracker.teardown_required !== null && workTracker.teardown_required !== undefined && (
            <View style={styles.flagRow}>
              <Ionicons name={workTracker.teardown_required ? 'construct-outline' : 'checkmark-circle-outline'} size={14} color={workTracker.teardown_required ? '#FF9500' : '#8E8E93'} />
        ...
        <View style={styles.section}>
          ...
          <TouchableOpacity onPress={() => openInMaps(dropoffAddress ? `${dropoffAddress.street}, ${dropoffAddress.city}, ${dropoffAddress.state_province}, ${dropoffAddress.zip_postal}` : undefined)}>
            <Text style={styles.addressText}>{dropoffAddress ? dropoffAddress.street : 'Address not set'}</Text>
          </TouchableOpacity>
          {workTracker.dropoff_time && <Text style={styles.detailText}>Time: {formatTime(workTracker.dropoff_time)}</Text>}
          {workTracker.dropoff_poc && <Text style={styles.detailText}>POC: {workTracker.dropoff_poc}</Text>}
          {workTracker.setup_required !== null && workTracker.setup_required !== undefined && (
            <View style={styles.flagRow}>
              <Ionicons name={workTracker.setup_required ? 'construct-outline' : 'checkmark-circle-outline'} size={14} color={workTracker.setup_required ? '#FF9500' : '#8E8E93'} />
...
const styles = StyleSheet.create({
  ...
  inspectionItem: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingVertical: 10, borderBottomWidth: 1, borderBottomColor: '#F2F2F7' },
  inspectionLabel: { fontSize: 15, fontWeight: '500', color: '#000' },
  inspectionValueContainer: { flexDirection: 'row', alignItems: 'center' },
