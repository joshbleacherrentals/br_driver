...
export const TripsList = observer(() => {
  const enrichedWorkTrackers = _enrichedWorkTrackers$.get();

  const handleStartTrip = (uuid: string) => {
    workTrackers$[uuid].status.set("in_progress");
  };

  // Helper to format address, handling empty fields
  const formatAddress = (address: EnrichedWorkTracker["pickup_address"]) => {
    if (!address) return undefined;
    const parts = [address.street, address.city, address.state_province, address.zip_postal].filter(
      (p) => p && p.trim() !== ""
    );
