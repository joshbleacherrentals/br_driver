...
export function useCheckDriver(): { driverProfile: boolean | undefined; isLoading: boolean } {
  ...
  const driverQueryResult = useTypedQuery(compiledDriver, expect<{ id: string; user_uuid: string | null, is_active: number | null}>());
  ...
  if (compiledDriver === null) return { driverProfile: undefined, isLoading: true };
  return {
    driverProfile: (driverQueryResult.data?.length ?? 0) > 0 && driverQueryResult.data?.[0]?.is_active === 1,
    isLoading: false,
  };
}