...
const UPCOMING_STATUSES = new Set([
  'released', 'accepted', 'dest_pickup',
  'pickup_inspection', 'dest_dropoff', 'dropoff_inspection',
]);
...
function formatStatus(status: string | null): string {
  ...
  const map: Record<string, string> = {
    released: 'Released', accepted: 'Accepted', dest_pickup: 'En Route',
    pickup_inspection: 'At Pickup', dest_dropoff: 'To Dropoff', dropoff_inspection: 'At Dropoff',
  };
...
export default function AvailabilityCalendarScreen() {
  ...
  const tripDotsByDate = useMemo(() => {
    ...
    for (const t of workTrackers ?? []) {
      ...
      else if (t.status && UPCOMING_STATUSES.has(t.status)) map[t.date].upcoming = true;
    }
    return map;
  }, [workTrackers]);
  ...
  const markedDates = useMemo<MarkedDates>(() => {
    ...
    const ensure = (d: string) => { if (!result[d]) result[d] = { dots: [] }; };

    ensure(TODAY);
    (result[TODAY].dots ??= []).push({ key: 'today', color: TODAY_BLUE });

    for (const [date, { upcoming, completed }] of Object.entries(tripDotsByDate)) {
      ensure(date);
  ...
  const datesInCurrentMonth = useCallback(
    (set: Set<string>) =>
      Array.from(set).filter(d => d.startsWith(currentMonth) && !isPastDate(d)),
    [currentMonth]
  );
  ...
  const handleDayPress = useCallback((day: DateData) => {
    if (isPastDate(day.dateString)) {
      Alert.alert('Past Date', 'You cannot modify availability for past dates.');
      return;
    }
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: () => {
            ...
            showUndoToast(`Cleared ${toRemove.length} date${toRemove.length > 1 ? 's' : ''} — tap Undo to restore`);
          },
        },
      ]
    );
  }, [localUnavailable, currentMonth, datesInCurrentMonth, pushUndo, showUndoToast]);
  ...
  }, [hasPendingChanges, localUnavailable, dbSavedSet, pushUndo, showUndoToast]);
  ...
  const handleSave = useCallback(async () => {
    ...
    if (!driverUuid) {
      Alert.alert('Error', 'Could not determine your driver profile. Please try again.');
      return;
    }
    ...
    try {
      ...
      for (const date of toRemove) {
        const row = dbUnavailableDates?.find(r => r.date_unavailable === date);
        if (!row) continue;
        const query = db
  ...
  }, [hasPendingChanges, pendingAdd, pendingRemove, dbUnavailableDates, driver]);
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: DARK_BLUE }}>
        ...
        <View style={styles.headerActions}>
          ...
          <TouchableOpacity
            style={[
              ...
              hasPendingChanges ? styles.headerSaveBtnDirty : styles.headerSaveBtnClean,
            ]}
            onPress={handleSave}
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.introStrip}>
          ...
          <Text style={styles.introText}>
            ...
            <Text style={{ color: PENDING_COLOR, fontWeight: '700' }}>Orange</Text> = unsaved,{' '}
            <Text style={{ color: SAVED_RED, fontWeight: '700' }}>red</Text> = saved.
          </Text>
        </View>
        ...
        <View style={[
          styles.statusBanner,
          futureCountThisMonth > 0 ? styles.statusUnavail : styles.statusAvail,
        ]}>
          <View style={[
            styles.statusDot,
            { backgroundColor: futureCountThisMonth > 0 ? SAVED_RED : SUCCESS_GREEN },
          ]} />
          <Text style={[
            styles.statusText,
        ...
        <View style={styles.legend}>
          <LegendItem color={SAVED_RED} label="Unavailable (saved)" shape="square" />
        ...
        {upcomingTrips.length > 0 && (
          <View style={styles.listSection}>
            <View style={styles.listSectionHeader}>
              <View style={[styles.sectionPill, { backgroundColor: DOT_UPCOMING + '22', borderColor: DOT_UPCOMING + '55' }]}>
                <View style={[styles.sectionPillDot, { backgroundColor: DOT_UPCOMING }]} />
                <Text style={[styles.sectionPillText, { color: DOT_UPCOMING }]}>Upcoming Trips</Text>
              </View>
              <Text style={styles.sectionCount}>{upcomingTrips.length}</Text>
            </View>

            {upcomingTrips.map((trip) => {
              const conflictDate = !!(trip.date && localUnavailable.has(trip.date));
              return (
                <View key={trip.id} style={[styles.tripCard, conflictDate && styles.tripCardConflict]}>
                  <View style={[styles.tripCardAccent, { backgroundColor: conflictDate ? SAVED_RED : DOT_UPCOMING }]} />
                  <View style={styles.tripCardBody}>
                    <View style={styles.tripCardRow}>
                      <Text style={styles.tripCardDate}>{formatDisplayDate(trip.date!)}</Text>
                  ...
                  <View style={[styles.statusBadge, { backgroundColor: DOT_UPCOMING + '25', borderColor: DOT_UPCOMING + '55' }]}>
      ...
      {undoToast !== null && (
        <View style={styles.undoToast} pointerEvents="box-none">
          <Text style={styles.undoToastText} numberOfLines={2}>{undoToast}</Text>
          <TouchableOpacity
            onPress={handleUndo}
...
function LegendItem({ color, label, shape }: { color: string; label: string; shape: 'dot' | 'square' }) {
  return (
...
const styles = StyleSheet.create({
  ...
  clearText: { fontSize: 12, color: '#fca5a5', fontWeight: '600', textDecorationLine: 'underline' },
  calendarCard: {
    borderRadius: 12,
    overflow: 'hidden',
    borderWidth: 1,
  ...
  sectionPillText: { fontSize: 12, fontWeight: '700', textTransform: 'uppercase', letterSpacing: 0.5 },
  ...
  conflictBadgeText: { fontSize: 11, color: '#FEF08A', fontWeight: '500', maxWidth: 200 },
